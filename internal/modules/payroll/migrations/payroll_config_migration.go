...
// CreatePayrollConfigTable creates the payroll_config table
func CreatePayrollConfigTable() error {
	db := database.GetDB()
	
	// Auto migrate the table structure
	if err := db.AutoMigrate(&models.PayrollConfig{}); err != nil {
		return fmt.Errorf("failed to create payroll_config table: %w", err)
	}
	
	// Check if config already exists
	var count int64
	db.Model(&models.PayrollConfig{}).Count(&count)
	if count > 0 {
		fmt.Println("Payroll config already seeded, skipping...")
		return nil
	}
	
	// Seed Tanzania statutory rates
	return seedTanzaniaStatutoryRates(db)
}

// seedTanzaniaStatutoryRates seeds the default Tanzania statutory rates
func seedTanzaniaStatutoryRates(db *gorm.DB) error {
	now := time.Now()
	...
	configurations := []models.PayrollConfig{
		...
		},
		{
			...
			Description: "10% of Gross Salary paid by employer. Formula from file: =H*10% Not deducted from employee net pay. Paid to: NSSF Tanzania",
			Category:    models.ConfigCategoryNSSF,
			Editable:    false,
			CreatedAt:   now,
			UpdatedAt:   now,
		...
		// WCF Rates
		{
			...
			Description: "0.5% of Gross Salary paid by employer only. Formula from file: =H*0.5% Not deducted from employee. Paid to: Workers Compensation Fund Tanzania",
		...
		// SDL Rates
		{
			...
			Description: "3.5% of Gross Salary paid by employer only. Formula from file: =H*3.5% Not deducted from employee. Paid to: Tanzania Revenue Authority (TRA)",
		...
		// HESLB Rates
		{
			...
			Description: "15% of Basic Pay deducted from employee. Formula from file: =BASIC_PAY*15% Applied only to employees with HESLB flag = true. Paid to: Higher Education Students Loans Board",
		...
		// Formulas
		{
			...
			Description: "Formula from file: =Gross − NSSF_Employee (10%). Taxable Pay is the base used for PAYE calculation.",
		...
		},
		{
			...
			Description: "Formula from file: =Taxable Pay − PAYE − HESLB − Loan. This is the final amount paid to the employee.",
			Category:    models.ConfigCategoryFormula,
			Editable:    false,
			CreatedAt:   now,
			UpdatedAt:   now,
		},
		{
			...
			Description: "Formula from file: =Gross + NSSF_Employer + WCF + SDL. Full cost GTL bears per employee per month.",
			Category:    models.ConfigCategoryFormula,
			Editable:    false,
			CreatedAt:   now,
			UpdatedAt:   now,
		...
		},
		{
			...
			Description: "GTL runs payroll once per calendar month. One payroll run per month per company.",
			Category:    models.ConfigCategoryGeneral,
			Editable:    false,
			CreatedAt:   now,
			UpdatedAt:   now,
	...
	if err := db.CreateInBatches(&configurations, 100).Error; err != nil {
		return fmt.Errorf("failed to seed payroll configurations: %w", err)
	}

	fmt.Printf("Successfully seeded %d Tanzania statutory payroll configurations\n", len(configurations))
	return nil
}